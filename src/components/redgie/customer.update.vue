    ...
    <v-container fluid>
      <v-row align="center" justify="center">
        <div class="pl-2 pr-2">
          ...
          <!-- username, fname, lname, gender, mobileno, addess, birthdate -->
          <div v-show="phase1" class="text-center">
            ...
            <v-row class="pa-0">
              <v-col cols="5" class="pa-1">
                <v-btn
                  dark
                  @click.stop="$router.push({name:'account'})"
          ...
          <div v-show="!phase1" class="text-center">
            ...
            <v-row class="pa-2">
              ...
              <v-col cols="7" class="pa-1">
                ...
                >
                  save
                  <v-icon size="40" class="mx-2">mdi mdi-database</v-icon>
                </v-btn>
              </v-col>
            </v-row>
          </div>
...
export default {
  ...
  mounted() {
    ...
    this.mobileno = this.user.mobileno.substring(1);
    this.gender = this.user.gender;
    this.birthdate = new Date(this.user.birthdate).toISOString().substr(0, 10);
  },
  methods: {
    handleConfirm() {
    ...
    submitPhase1(e) {
      ...
      try {
        ...
        this.$store.dispatch("SET_HEADER", "Complete Updates");
        this.phase1 = false;
        window.scrollTo(0, 0);
      } catch (error) {
    ...
    handleSubmit() {
      try {
        ...
        if (!this.completionForm)
          throw "Complete Updates has invalid input. Please check your input.";
        ...
        auth
          ...
          .then(result => {
            console.log(result);
            this.$store.dispatch("onUpdateUser", result.body.user).then(res => {
